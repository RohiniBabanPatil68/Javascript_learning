console.warn("-----------------Even Position Characters--------------------------");
...
evenPositionedChars("Hard work always pays back");
   ...
   console.warn("-----------Odd Numbers------------");
 
   function string(givenString02) {
      
      var totalChar = givenString02.length ;  
      
      for (let index = 0; index <= totalChar; index++) {
          if (index%2!=0 && index !="") {
              var charAt=givenString02.charAt(index);
              console.log(charAt);
          }
      }
      }
